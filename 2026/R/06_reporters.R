...
import_rsf <- function(year) {
  ...
  locale <- if (year >= 2025) {
    readr::locale(decimal_mark = ",", encoding = "Windows-1252")
  } else {
...
# This old useless code
# dumbbell <- ggplot(plot_data, aes(y = label_y)) +
#   geom_segment(
#     aes(x = score_2022, xend = score_2026, yend = label_y,
#         color = direction),
#     linewidth = 1
#   ) +
...
#   geom_text(
#     data = df_ends,
#     aes(x = score_2026 + label_off, label = label_score,
#         hjust = label_h, color = direction),
#     family = font_text, size = 2.6, show.legend = FALSE
#   ) +
#   scale_x_continuous(
#     limits = c(20, 92),
#     breaks = seq(20, 90, 10),
