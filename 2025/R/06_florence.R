# Prompt: Comparisons — Florence Nightingale (rose diagram)
# Monthly movie-theater ticket sales in Brazil, 2019 vs 2024.
# Source: ANCINE (2025). Note: lubridate::month() is kept qualified to avoid
# masking data.table::month() used in the data.table chains below.
...
files_path <- dir_ls(dirpath)
files <- mclapply(files_path, fread)
# Stack all datasets
...
# Export
ggsave(
  here("2025/plots/06_florence_nightingale.png"),
  final_plot,
  width = 8,
  height = 8 / 1.618
)
